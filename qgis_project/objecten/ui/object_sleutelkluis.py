...
def formOpen(dialog, layer, feature):
    ...
    try:
        if (feature.geometry()):
            ...
            nameField.append(dialog.findChild(QComboBox, "sleutelkluis_type_id"))            
            nameValidate.append(0)
            nameField.append(dialog.findChild(QComboBox, "sleuteldoel_type_id"))            
            nameValidate.append(0)
            ...
            if not (len(nameField[0].currentText()) > 0):
                ...
                nameValidate[0] = 1				
            if (feature['ingang_id'] == NULL):
                geom = feature.geometry().asPoint()
                extent = iface.mapCanvas().extent()
                objectenLayer = getVectorLayerByName("Ingang ruimtelijk")
                ...
                myDialog.changeAttribute("ingang_id", featureId)
            nameField[0].currentIndexChanged.connect(lambda: validate(nameField, nameValidate, okButton))
            nameField[1].currentIndexChanged.connect(lambda: validate(nameField, nameValidate, okButton))
